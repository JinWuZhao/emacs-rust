...
then
    ...
    if [ -f "/root/.emacs" ];
    then
        cp -f /root/.emacs $USER_HOME/
    fi
    if [ -d "/root/.emacs.d" ];
    then
        cp -rf /root/.emacs.d $USER_HOME/
    fi
    chown -R "$NAME:$NAME" $USER_HOME
fi
unset MY_UID
...
if [ -d "$RUST_HOME" ];
then  
    ln -s $RUST_HOME/rustup $USER_HOME/.rustup
    ln -s $RUST_HOME/cargo $USER_HOME/.cargo
...
then
    ...
    printf "#!/bin/bash\ncd /mnt/share/Documents && emacs -nw .\n" > /tmp/run_emacs.sh
    chmod +x /tmp/run_emacs.sh
    runuser -u $NAME /tmp/run_emacs.sh
fi
